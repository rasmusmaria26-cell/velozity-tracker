...
interface KanbanColumnProps {
    ...
    colorClass: string
    tasks: Task[]
    getCardProps: (task: Task) => {
        onPointerDown: (e: React.PointerEvent<HTMLDivElement>) => void
    }
}

const KanbanColumn = ({ status, label, colorClass, tasks, getCardProps }: KanbanColumnProps) => {

    return (
        <div className={`flex flex-col min-w-[280px] w-[280px] bg-slate-50/50 rounded-xl border-t-4 ${colorClass} shadow-sm overflow-hidden`}>
